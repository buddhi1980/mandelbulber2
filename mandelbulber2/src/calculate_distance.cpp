	...
	const sDistanceIn &in, sDistanceOut *out, sRenderData *data)
{
	...
	// NOTE: the old per-fractal boolean-operator path below has been superseded by the
	// objects-tree implementation in CalculateDistanceFromObjectsTree(). Per-fractal
	// transform data (position/rotation/repeat/scale) is now stored in cObjectData and
