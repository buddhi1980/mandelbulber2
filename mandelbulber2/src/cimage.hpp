...
class cImage {
 public:
  ...
  inline sRGBfloat GetPixelNormal(int x, int y) {
    ...
    if (x >= 0 && x < width && y >= 0 && y < height)
      return normalFloat[x + y * width];
    return BlackFloat();
  }
  inline sRGB16 GetPixelNormal16(int x, int y) {
    if (!opt.optionalNormal) return Black16();
    if (x >= 0 && x < width && y >= 0 && y < height)
      return normal16[x + y * width];
    return Black16();
  }
  inline sRGB8 GetPixelNormal8(int x, int y) {
    if (!opt.optionalNormal) return Black8();
    if (x >= 0 && x < width && y >= 0 && y < height)
      return normal8[x + y * width];
    return Black8();
  }
  inline void BlendPixelImage16(int x, int y, double factor, sRGB16 other) {
    double factorN = 1.0 - factor;
    image16[x + y * width].R =
        image16[x + y * width].R * factorN + other.R * factor;
    image16[x + y * width].G =
        image16[x + y * width].G * factorN + other.G * factor;
    image16[x + y * width].B =
        image16[x + y * width].B * factorN + other.B * factor;
  }

  inline void BlendPixelImage(int x, int y, double factor, sRGBfloat other) {
    double factorN = 1.0 - factor;
    imageFloat[x + y * width].R =
        imageFloat[x + y * width].R * factorN + other.R * factor;
    imageFloat[x + y * width].G =
        imageFloat[x + y * width].G * factorN + other.G * factor;
    imageFloat[x + y * width].B =
        imageFloat[x + y * width].B * factorN + other.B * factor;
  }

  inline void BlendPixelAlpha(int x, int y, double factor,
                              unsigned short int other) {
    double factorN = 1.0 - factor;
    alphaBuffer16[x + y * width] =
        alphaBuffer16[x + y * width] * factorN + other * factor;
  }
  ...
  unsigned char *CreatePreview(double scale, int visibleWidth,
                               int visibleHeight, QWidget *widget);
  ...
  void PutPixelAlfa(int x, int y, float z, sRGB8 color, double opacity,
                    int layer);
  void AntiAliasedPoint(double x, double y, float z, sRGB8 color,
                        double opacity, int layer);
  void AntiAliasedLine(double x1, double y1, double x2, double y2, float z1,
                       float z2, sRGB8 color, double opacity, int layer);
  void CircleBorder(double x, double y, float z, double r, sRGB8 border,
                    double borderWidth, double opacity, int layer);
...
sRGB PostRendering_Fog(double z, double min, double max, sRGB fog_color,
                       sRGB color_before);
