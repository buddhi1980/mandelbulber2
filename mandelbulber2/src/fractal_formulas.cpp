...
void AboxMod12Iteration(CVector4 &z, const sFractal *fractal, sExtendedAux &aux)
{
	...
	if (fractal->transformCommon.functionEnabledAx)
	{
				...
				&& aux.i < fractal->transformCommon.stopIterationsS)
		{
			...
			CVector4 limitMinR2 = fractal->transformCommon.additionConstant0555;
			double m = 1.0;
			double rr = z.Dot(z);
	...
	else
	{
				...
				&& aux.i < fractal->transformCommon.stopIterationsS)
		{
			...
			if (rr < minR2)
			{
				double tglad_factor1 = fractal->transformCommon.maxR2d1 / minR2;
				z *= tglad_factor1;
				aux.DE *= tglad_factor1;
				aux.color += fractal->mandelbox.color.factorSp1;
			}
	...
	double scaleColor = fractal->foldColor.colorMin + fabs(useScale); // scale, useScale, m, etc
	// scaleColor += fabs(fractal->mandelbox.scale);
	aux.scaleFactor = scaleColor * fractal->foldColor.compScale;
}
...
void TransfHybridColorIteration(CVector4 &z, const sFractal *fractal, sExtendedAux &aux)
{
	...
	// limits components
	if (components < fractal->transformCommon.scale025)
		components = fractal->transformCommon.scale025 * 1000000;
	if (components > fractal->transformCommon.scale8)
		components = fractal->transformCommon.scale8 * 1000000;
