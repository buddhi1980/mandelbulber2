 ...
 *
 * functions for calculation fractal transforma
 *
...
void additionConstantTransform3D(const sTransformAdditionConstant &additionConstant, CVector3 &z,
		int i)
{
	if (additionConstant.control.enabled && i >= additionConstant.control.startIterations
			&& i < additionConstant.control.stopIterations)
	{
		CVector3 temp = z;
...
void additionConstantTransform4D(const sTransformAdditionConstant &additionConstant, CVector4 &z,
		int i)
{
	if (additionConstant.control.enabled && i >= additionConstant.control.startIterations
			&& i < additionConstant.control.stopIterations)
...
{
	if (boxConstantMultiplier.control.enabled && i >= boxConstantMultiplier.control.startIterations
			&& i < boxConstantMultiplier.control.stopIterations)
	{
		CVector3 temp = z;
		CVector3 tempFAB = c;
...
void boxFoldOriginalTransform3D(const sTransformBoxFoldOriginal &boxFoldOriginal, CVector3 &z,
		int i, sExtendedAux &aux)
{
	if (boxFoldOriginal.control.enabled && i >= boxFoldOriginal.control.startIterations
			&& i < boxFoldOriginal.control.stopIterations)
	{
		CVector3 temp = z;
...
{
	if (boxFold.control.enabled && i >= boxFold.control.startIterations
			&& i < boxFold.control.stopIterations)
	{
		CVector3 temp = z;
...
void boxOffsetTransform3D(const sTransformBoxOffset &boxOffset, CVector3 &z, int i,
		sExtendedAux &aux)
{
	if (boxOffset.control.enabled && i >= boxOffset.control.startIterations
			&& i < boxOffset.control.stopIterations)
	{
		CVector3 temp = z;
		if (z.x > 0)
		{
...
void constantMultiplierOriginalTransform3D(
		const sTransformConstantMultiplierOriginal &constantMultiplierOriginal, CVector3 &z,
		CVector3 &c, int i)
{
	if (constantMultiplierOriginal.control.enabled
			&& i >= constantMultiplierOriginal.control.startIterations
			&& i < constantMultiplierOriginal.control.stopIterations)
	{
		CVector3 temp = z;
...
void constantMultiplierOriginalTransform4D(
		const sTransformConstantMultiplierOriginal &constantMultiplierOriginal, CVector4 &z,
		const CVector4 &c, int i)
{
	if (constantMultiplierOriginal.control.enabled
			&& i >= constantMultiplierOriginal.control.startIterations
			&& i < constantMultiplierOriginal.control.stopIterations)
...
void constantMultiplierTransform3D(const sTransformConstantMultiplier &constantMultiplier,
		CVector3 &z, CVector3 &c, int i)
{
	if (constantMultiplier.control.enabled && i >= constantMultiplier.control.startIterations
			&& i < constantMultiplier.control.stopIterations)
	{
		CVector3 temp = z;
...
void constantMultiplierTransform4D(const sTransformConstantMultiplier &constantMultiplier,
		CVector4 &z, const CVector4 &c, int i)
{
	if (constantMultiplier.control.enabled && i >= constantMultiplier.control.startIterations
			&& i < constantMultiplier.control.stopIterations)
	{
...
void fabsAddConstantTransform3D(const sTransformFabsAddConstant &fabsAddConstant, CVector3 &z,
		int i)
{
	if (fabsAddConstant.control.enabled && i >= fabsAddConstant.control.startIterations
			&& i < fabsAddConstant.control.stopIterations)
	{
		CVector3 temp = z;
...
void fabsAddConstantTransform4D(const sTransformFabsAddConstant &fabsAddConstant, CVector4 &z,
		int i)
{
	if (fabsAddConstant.control.enabled && i >= fabsAddConstant.control.startIterations
			&& i < fabsAddConstant.control.stopIterations)
...
{
	if (fabsFormulaAB.control.enabled && i >= fabsFormulaAB.control.startIterations
			&& i < fabsFormulaAB.control.stopIterations)
	{
		CVector3 temp = z;
		if (fabsFormulaAB.fabsFormulaABEnabledx)
		{
			z.x = fabs(z.x + fabsFormulaAB.fabsFormulaABConstantA.x)
					- fabs(z.x - fabsFormulaAB.fabsFormulaABConstantB.x) - z.x;
		}
		if (fabsFormulaAB.fabsFormulaABEnabledy)
		{
			z.y = fabs(z.y + fabsFormulaAB.fabsFormulaABConstantA.y)
					- fabs(z.y - fabsFormulaAB.fabsFormulaABConstantB.y) - z.y;
		}
		if (fabsFormulaAB.fabsFormulaABEnabledz)
		{
			z.z = fabs(z.z + fabsFormulaAB.fabsFormulaABConstantA.z)
					- fabs(z.z - fabsFormulaAB.fabsFormulaABConstantB.z) - z.z;
		}
		//old weight function
...
void fabsFormulaABCDTransform3D(const sTransformFabsFormulaABCD &fabsFormulaABCD, CVector3 &z,
		int i)
{
	if (fabsFormulaABCD.control.enabled && i >= fabsFormulaABCD.control.startIterations
			&& i < fabsFormulaABCD.control.stopIterations)
	{
		CVector3 temp = z;
		CVector3 tempA = z * 0;
		CVector3 tempB = z * 0;
		...
		z.x = tempA.x - tempB.x
				+ (z.x * fabsFormulaABCD.fabsFormulaABCDC.x + fabsFormulaABCD.fabsFormulaABCDD.x);
		...
		z.y = tempA.y - tempB.y
				+ (z.y * fabsFormulaABCD.fabsFormulaABCDC.y + fabsFormulaABCD.fabsFormulaABCDD.y);
		...
		z.z = tempA.z - tempB.z
				+ (z.z * fabsFormulaABCD.fabsFormulaABCDC.z + fabsFormulaABCD.fabsFormulaABCDD.z);
...
void fabsFormulaABCDTransform4D(const sTransformFabsFormulaABCD &fabsFormulaABCD, CVector4 &z,
		int i)
{
	if (fabsFormulaABCD.control.enabled && i >= fabsFormulaABCD.control.startIterations
			&& i < fabsFormulaABCD.control.stopIterations)
	{
		...
		z.x = tempA.x - tempB.x
				+ (z.x * fabsFormulaABCD.fabsFormulaABCD4DC.x + fabsFormulaABCD.fabsFormulaABCD4DD.x);
		if (fabsFormulaABCD.fabsFormulaABCDEnabledAy)
		{
		...
		z.y = tempA.y - tempB.y
				+ (z.y * fabsFormulaABCD.fabsFormulaABCD4DC.y + fabsFormulaABCD.fabsFormulaABCD4DD.y);
		if (fabsFormulaABCD.fabsFormulaABCDEnabledAz)
		{
		...
		z.z = tempA.z - tempB.z
				+ (z.z * fabsFormulaABCD.fabsFormulaABCD4DC.z + fabsFormulaABCD.fabsFormulaABCD4DD.z);
		...
		z.z = tempA.w - tempB.z
				+ (z.w * fabsFormulaABCD.fabsFormulaABCD4DC.w + fabsFormulaABCD.fabsFormulaABCD4DD.w);
		//Old weight
		if (fabsFormulaABCD.control.oldWeightEnabled)
		{
			z = temp + ((z - temp) * (fabsFormulaABCD.control.oldWeight));
...
{
	if (fabsFormulaZAB.control.enabled && i >= fabsFormulaZAB.control.startIterations
			&& i < fabsFormulaZAB.control.stopIterations)
	{
		CVector3 temp = z;
...
void fabsSubConstantTransform3D(const sTransformFabsSubConstant &fabsSubConstant, CVector3 &z,
		int i)
{
	if (fabsSubConstant.control.enabled && i >= fabsSubConstant.control.startIterations
			&& i < fabsSubConstant.control.stopIterations)
	{
		CVector3 temp = z;
...
// iterationWeight 3D
void iterationWeightTransform3D(const sTransformIterationWeight &iterationWeight, CVector3 &z,
		int i)
{
	if (iterationWeight.control.enabled)
	{
		...
		z = (z * iterationWeight.iterationWeightConstantZ)
				+ (zA * iterationWeight.iterationWeightConstantA)
				+ (zB * iterationWeight.iterationWeightConstantB);
		//weight function
...
// iterationWeight 4D
void iterationWeightTransform4D(const sTransformIterationWeight &iterationWeight, CVector4 &z,
		int i)
{
	if (iterationWeight.control.enabled)
	{
...
void mandelbulbOriginalTransform3D(const sTransformMandelbulbOriginal &mandelbulbOriginal,
		CVector3 &z, int i, sExtendedAux &aux)
{
	if (mandelbulbOriginal.control.enabled && i >= mandelbulbOriginal.control.startIterations
			&& i < mandelbulbOriginal.control.stopIterations)
	{
		CVector3 temp = z;
		aux.r = z.Length();
...
// mandelbulb 3D
void mandelbulbTransform3D(const sTransformMandelbulb &mandelbulb, CVector3 &z, int i,
		sExtendedAux &aux)
{
	if (mandelbulb.control.enabled && i >= mandelbulb.control.startIterations
			&& i < mandelbulb.control.stopIterations)
	{
		CVector3 temp = z;
		aux.r = z.Length();
		if (aux.r < 1e-21) aux.r = 1e-21;
...
{
	if (mainRotation.control.enabled && i >= mainRotation.control.startIterations
			&& i < mainRotation.control.stopIterations)
	{
		CVector3 temp = z;
...
void mengerSpongeOriginalTransform3D(const sTransformMengerSpongeOriginal &mengerSpongeOriginal,
		CVector3 &z, int i, sExtendedAux &aux)
{
	if (mengerSpongeOriginal.control.enabled && i >= mengerSpongeOriginal.control.startIterations
			&& i < mengerSpongeOriginal.control.stopIterations)
	{
...
void mengerSpongeTransform3D(const sTransformMengerSponge &mengerSponge, CVector3 &z, int i,
		sExtendedAux &aux)
{
	if (mengerSponge.control.enabled && i >= mengerSponge.control.startIterations
			&& i < mengerSponge.control.stopIterations)
	{
		//double tempAuxDE = aux.ifsDE;
		CVector3 temp = z;
		double tempMS;
...
void quaternionOriginalTransform4D(const sTransformQuaternionOriginal &quaternionOriginal,
		CVector4 &z, int i)
{
	if (quaternionOriginal.control.enabled && i >= quaternionOriginal.control.startIterations
			&& i < quaternionOriginal.control.stopIterations)
	{
		CVector4 temp = z;
		CVector4 newz(quaternionOriginal.quaternionOriginalFactorConstant.x
...
{
	if (quaternion.control.enabled && i >= quaternion.control.startIterations
			&& i < quaternion.control.stopIterations)
	{
		CVector4 temp = z;
		CVector4 newz(quaternion.quaternionFactorConstant.x
											* (z.x * z.x - z.y * z.y - z.z * z.z - z.w * z.w),
...
void scaleOriginalTransform3D(const sTransformScaleOriginal &scaleOriginal, CVector3 &z, int i,
		sExtendedAux &aux)
{
	if (scaleOriginal.control.enabled && i >= scaleOriginal.control.startIterations
			&& i < scaleOriginal.control.stopIterations)
	{
...
{
	if (scale.control.enabled && i >= scale.control.startIterations
			&& i < scale.control.stopIterations)
	{
		z = z * scale.scale;
...
void sphericalFoldOriginalTransform3D(const sTransformSphericalFoldOriginal &sphericalFoldOriginal,
		CVector3 &z, int i, sExtendedAux &aux)
{
	if (sphericalFoldOriginal.control.enabled && i >= sphericalFoldOriginal.control.startIterations
			&& i < sphericalFoldOriginal.control.stopIterations)
	{
		CVector3 temp = z;
...
void sphericalFoldTransform3D(const sTransformSphericalFold &sphericalFold, CVector3 &z, int i,
		sExtendedAux &aux)
{
	if (sphericalFold.control.enabled && i >= sphericalFold.control.startIterations
			&& i < sphericalFold.control.stopIterations)
	{
		CVector3 temp = z;
		double tempAuxDE = aux.DE;
		double r2 = z.Dot(z);
...
void sphericalFoldTransform4D(const sTransformSphericalFold &sphericalFold, CVector4 &z, int i,
		sExtendedAux &aux)
{
	if (sphericalFold.control.enabled && i >= sphericalFold.control.startIterations
			&& i < sphericalFold.control.stopIterations)
...
void sphericalOffsetTransform3D(const sTransformSphericalOffset &sphericalOffset, CVector3 &z,
		int i, sExtendedAux &aux)
{
	if (sphericalOffset.control.enabled && i >= sphericalOffset.control.startIterations
			&& i < sphericalOffset.control.stopIterations)
	{
		CVector3 temp = z;
		double tempAuxDE = aux.DE;
...
void variableConstantMultiplierTransform3D(
		const sTransformVariableConstantMultiplier &variableConstantMultiplier, CVector3 &z,
		CVector3 &c, int i)
{
	if (variableConstantMultiplier.control.enabled
			&& i >= variableConstantMultiplier.control.startIterations
			&& i < variableConstantMultiplier.control.stopIterations)
	{
		CVector3 temp = z;
		...
		if (i >= variableConstantMultiplier.variableStartIterations
				&& i < variableConstantMultiplier.variableStopIterations
				&& (variableConstantMultiplier.variableStopIterations
						- variableConstantMultiplier.variableStartIterations != 0))
		{
			tempVC = (tempVC
					+ variableConstantMultiplier.variableConstant
							* (i - variableConstantMultiplier.variableStartIterations)
							/ (variableConstantMultiplier.variableStopIterations
									- variableConstantMultiplier.variableStartIterations));
		}
...
void variableMandelbulbPowerTransform3D(
		const sTransformVariableMandelbulbPower &variableMandelbulbPower, CVector3 &z, int i,
		sExtendedAux &aux)
{
	if (variableMandelbulbPower.control.enabled
			&& i >= variableMandelbulbPower.control.startIterations
			&& i < variableMandelbulbPower.control.stopIterations)
	{
		CVector3 temp = z;
		...
		if (i >= variableMandelbulbPower.variableStartIterations
				&& i < variableMandelbulbPower.variableStopIterations
				&& (variableMandelbulbPower.variableStopIterations
						- variableMandelbulbPower.variableStartIterations != 0))
		{
			tempVC = (tempVC
					+ variableMandelbulbPower.variableConstant
							* (i - variableMandelbulbPower.variableStartIterations)
							/ (variableMandelbulbPower.variableStopIterations
									- variableMandelbulbPower.variableStartIterations));
		}
...
void variableScaleTransform3D(const sTransformVariableScale &variableScale, CVector3 &z, int i,
		sExtendedAux &aux)
{
	if (variableScale.control.enabled && i >= variableScale.control.startIterations
			&& i < variableScale.control.stopIterations)
	{
		...
		if (i >= variableScale.variableStartIterations && i < variableScale.variableStopIterations
				&& (variableScale.variableStopIterations - variableScale.variableStartIterations != 0))
		{
			tempVC = (tempVC
					+ variableScale.variableConstant * (i - variableScale.variableStartIterations)
							/ (variableScale.variableStopIterations - variableScale.variableStartIterations));
		}
...
void coloringParametersTransform3D(const sTransformColoringParameters &coloringParameters,
		CVector3 &z, double minimumR, int i, sExtendedAux &aux)
{
	if (coloringParameters.control.enabled && i >= coloringParameters.control.startIterations
			&& i < coloringParameters.control.stopIterations)
	{
		double R;
		R = z.Length();
...
void colorTrialTransform3D(const sTransformColorTrial &colorTrial, CVector3 &z, int i,
		sExtendedAux &aux)
{
	if (colorTrial.control.enabled && i >= colorTrial.control.startIterations
			&& i < colorTrial.control.stopIterations)
...
// transform sampling
		...
		double R = (aux.axisBias + (aux.orbitTraps * colorTrial.mainOrbitTrapWeight)) * 1000
				* colorTrial.minimumRWeight;
...
// mandelbulbMulti 3D
void mandelbulbMultiTransform3D(const sTransformMandelbulbMulti &mandelbulbMulti, CVector3 &z,
		int i, sExtendedAux &aux)
{
	CVector3 temp = z;
	aux.r = z.Length();
	if (mandelbulbMulti.control.enabled && i >= mandelbulbMulti.control.startIterations
			&& i < mandelbulbMulti.control.stopIterations)
	{
		...
		{
			case sTransformMandelbulbMulti::xyz:
			default:
				v1 = z.x;
				v2 = z.y;
				v3 = z.z;
				break;
			case sTransformMandelbulbMulti::xzy:
				v1 = z.x;
				v2 = z.z;
				v3 = z.y;
				break;
			case sTransformMandelbulbMulti::yxz:
				v1 = z.y;
				v2 = z.x;
				v3 = z.z;
				break;
			case sTransformMandelbulbMulti::yzx:
				v1 = z.y;
				v2 = z.z;
				v3 = z.x;
				break;
			case sTransformMandelbulbMulti::zxy:
				v1 = z.z;
				v2 = z.x;
				v3 = z.y;
				break;
			case sTransformMandelbulbMulti::zyx:
				v1 = z.z;
				v2 = z.y;
				v3 = z.x;
				break;
...
void benesiMagForwardTransformOneTransform3D(
		const sTransformBenesiMagForwardTransformOne &benesiMagForwardTransformOne, CVector3 &z, int i)
{
	if (benesiMagForwardTransformOne.control.enabled
			&& i >= benesiMagForwardTransformOne.control.startIterations
			&& i < benesiMagForwardTransformOne.control.stopIterations)
	{
		CVector3 temp = z;
...
void benesiMagBackTransformOneTransform3D(
		const sTransformBenesiMagBackTransformOne &benesiMagBackTransformOne, CVector3 &z, int i)
{
	if (benesiMagBackTransformOne.control.enabled
			&& i >= benesiMagBackTransformOne.control.startIterations
			&& i < benesiMagBackTransformOne.control.stopIterations)
	{
		CVector3 temp = z;
...
void benesiMagTransformOneTransform3D(const sTransformBenesiMagTransformOne &benesiMagTransformOne,
		CVector3 &z, int i, sExtendedAux &aux)
{
	if (benesiMagTransformOne.control.enabled && i >= benesiMagTransformOne.control.startIterations
			&& i < benesiMagTransformOne.control.stopIterations)
	{
		CVector3 temp = z;
		double tempXZ = z.x * SQRT_2_3 - z.z * SQRT_1_3;
		z = CVector3((tempXZ - z.y) * SQRT_1_2,
...
//  Change this for different transforms
		...
		double avgScale = (fabs(benesiMagTransformOne.scale.x) + fabs(benesiMagTransformOne.scale.y)
				+ fabs(benesiMagTransformOne.scale.z)) / 3; // cheap approximation
		aux.r_dz *= avgScale;
...
void benesiMagTransformOneEnabledTransform3D(
		const sTransformBenesiMagTransformOneEnabled &benesiMagTransformOneEnabled, CVector3 &z, int i,
		sExtendedAux &aux)
{
	if (benesiMagTransformOneEnabled.control.enabled
			&& i >= benesiMagTransformOneEnabled.control.startIterations
			&& i < benesiMagTransformOneEnabled.control.stopIterations)
	{
		...
		double avgScale = (fabs(benesiMagTransformOneEnabled.scale.x)
				+ fabs(benesiMagTransformOneEnabled.scale.y) + fabs(benesiMagTransformOneEnabled.scale.z))
...
void benesiMagTransformOnePlusMinusTransform3D(
		const sTransformBenesiMagTransformOnePlusMinus &benesiMagTransformOnePlusMinus, CVector3 &z,
		int i, sExtendedAux &aux)
{
	if (benesiMagTransformOnePlusMinus.control.enabled
			&& i >= benesiMagTransformOnePlusMinus.control.startIterations
			&& i < benesiMagTransformOnePlusMinus.control.stopIterations)
	{
		CVector3 temp = z;
		double tempXZ = z.x * SQRT_2_3 - z.z * SQRT_1_3;
		z = CVector3((tempXZ - z.y) * SQRT_1_2,
...
//  Change this for different transforms
		...
		z = (fabs(z + benesiMagTransformOnePlusMinus.offset)
				- fabs(z - benesiMagTransformOnePlusMinus.offset) - z);

		double avgScale = (fabs(benesiMagTransformOnePlusMinus.scale.x)
				+ fabs(benesiMagTransformOnePlusMinus.scale.y)
				+ fabs(benesiMagTransformOnePlusMinus.scale.z)) / 3;  // cheap approximation
		aux.r_dz *= avgScale;
...
void benesiMagTransformTwoTransform3D(const sTransformBenesiMagTransformTwo &benesiMagTransformTwo,
		CVector3 &z, int i, sExtendedAux &aux)
{
	if (benesiMagTransformTwo.control.enabled && i >= benesiMagTransformTwo.control.startIterations
			&& i < benesiMagTransformTwo.control.stopIterations)
	{
		CVector3 temp = z;
		double tempXZ = z.x * SQRT_2_3 - z.z * SQRT_1_3;
		z = CVector3((tempXZ - z.y) * SQRT_1_2,
...
//  Change this for different transforms
		...
		double avgScale = (fabs(benesiMagTransformTwo.scale.x) + fabs(benesiMagTransformTwo.scale.y)
				+ fabs(benesiMagTransformTwo.scale.z)) / 3; // cheap approximation
		//aux.r_dz *= avgScale;
		aux.DE = aux.DE * fabs(avgScale) + 1.0;
...
void benesiMagTransformThreeTransform3D(
		const sTransformBenesiMagTransformThree &benesiMagTransformThree, CVector3 &z, int i)
{
	if (benesiMagTransformThree.control.enabled
			&& i >= benesiMagTransformThree.control.startIterations
			&& i < benesiMagTransformThree.control.stopIterations)
	{
		CVector3 temp = z;
		double tempXZ = z.x * SQRT_2_3 - z.z * SQRT_1_3;
		z = CVector3((tempXZ - z.y) * SQRT_1_2,
...
void benesiMagTransformFourTransform3D(
		const sTransformBenesiMagTransformFour &benesiMagTransformFour, CVector3 &z, int i)
{
	if (benesiMagTransformFour.control.enabled && i >= benesiMagTransformFour.control.startIterations
			&& i < benesiMagTransformFour.control.stopIterations)
	{
		CVector3 temp = z;
		double tempXZ = z.x * SQRT_2_3 - z.z * SQRT_1_3;
		z = CVector3((tempXZ - z.y) * SQRT_1_2,
...
void benesiMagTransformFiveBTransform3D(
		const sTransformBenesiMagTransformFiveB &benesiMagTransformFiveB, CVector3 &z, int i)
{
	if (benesiMagTransformFiveB.control.enabled
			&& i >= benesiMagTransformFiveB.control.startIterations
			&& i < benesiMagTransformFiveB.control.stopIterations)
	{
		CVector3 temp = z;
		double tempXZ = z.x * SQRT_2_3 - z.z * SQRT_1_3;
		z = CVector3((tempXZ - z.y) * SQRT_1_2,
		...
		CVector3 tempV2 = z;
		tempV2.x = fabs(pow(pow(z.y, benesiMagTransformFiveB.powOne.x)
														+ pow(z.z, benesiMagTransformFiveB.powOne.x),
												benesiMagTransformFiveB.powTwo.x));
		tempV2.y = fabs(pow(pow(z.x, benesiMagTransformFiveB.powOne.y)
														+ pow(z.z, benesiMagTransformFiveB.powOne.y),
												benesiMagTransformFiveB.powTwo.y));
		tempV2.z = fabs(pow(pow(z.x, benesiMagTransformFiveB.powOne.z)
														+ pow(z.y, benesiMagTransformFiveB.powOne.z),
												benesiMagTransformFiveB.powTwo.z));
...
void benesiFastPwr2PineTreeTransform3D(
		const sTransformBenesiFastPwr2PineTree &benesiFastPwr2PineTree, CVector3 &z, CVector3 &c, int i,
		sExtendedAux &aux)
{
	if (benesiFastPwr2PineTree.control.enabled && i >= benesiFastPwr2PineTree.control.startIterations
			&& i < benesiFastPwr2PineTree.control.stopIterations)
	{
		CVector3 temp = z;
		aux.r = z.Length();
		z *= z;
...
void benesiFastPwr2PineTreeEnabledTransform3D(
		const sTransformBenesiFastPwr2PineTreeEnabled &benesiFastPwr2PineTreeEnabled, CVector3 &z,
		CVector3 &c, int i, sExtendedAux &aux)
{
	if (benesiFastPwr2PineTreeEnabled.control.enabled
			&& i >= benesiFastPwr2PineTreeEnabled.control.startIterations
			&& i < benesiFastPwr2PineTreeEnabled.control.stopIterations)
	{
		CVector3 temp = z;
		aux.r = z.Length();
		z *= z;
...
void boxFoldMultiTransform3D(const sTransformBoxFoldMulti &boxFoldMulti, CVector3 &z, int i,
		sExtendedAux &aux)
{
	if (boxFoldMulti.control.enabled && i >= boxFoldMulti.control.startIterations
			&& i < boxFoldMulti.control.stopIterations)
	{
		CVector3 temp = z;
...
{
	if (cubeSphere.control.enabled && i >= cubeSphere.control.startIterations
			&& i < cubeSphere.control.stopIterations)
	{
		CVector3 temp = z;
...
{
	if (sphereCube.control.enabled && i >= sphereCube.control.startIterations
