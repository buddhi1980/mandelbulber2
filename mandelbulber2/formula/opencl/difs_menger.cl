 ...
 * from the function "DIFSMengerIteration" in the file fractal_formulas.cpp
...
REAL4 DIFSMengerIteration(REAL4 z, __constant sFractalCl *fractal, sExtendedAuxCl *aux)
{
	...
	{
		REAL rr;
		REAL offsetPy = fractal->transformCommon.offset4;
		...
		// fabs() and menger fold
		{
			...
			// if (z.x - z.y < 0.0f) { REAL temp = z.y; z.y = z.x; z.x = temp; }
			// if (z.x - z.z < 0.0f) { REAL temp = z.z; z.z = z.x; z.x = temp; }
			// if (z.y - z.z < 0.0f) { REAL temp = z.z; z.z = z.y; z.y = temp; }
			...
			{
				if (zc.y < native_recip(offsetPy))
					zc.y = fabs(zc.y - native_recip(offsetPy)) + native_recip(offsetPy);
			}
			...
			//			if (fractal->transformCommon.functionEnabledM) {zc.y = fabs(zc.y - native_divide(3.0f,
			//offsetPy))
			//+ native_divide(3.0f, offsetPy);}
		...
		// if(any(greaterThan(p,vec3(0.f)))) r2=length(max(p,0.));
			...
			&& aux->i < fractal->transformCommon.stopIterationsM)
	{
		...
		REAL3 q = (REAL3){max(abs(zc.y) - lenY, 0.0f), max(abs(zc.x) - lenX, 0.0f), zc.z};
