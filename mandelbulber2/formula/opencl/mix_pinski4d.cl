...
float4 MixPinski4dIteration(float4 z, __constant sFractalCl *fractal, sExtendedAuxCl *aux)
{
...
#else
double4 MixPinski4dIteration(double4 z, __constant sFractalCl *fractal, sExtendedAuxCl *aux)
{
	if (aux->i >= fractal->transformCommon.startIterationsS
			&& aux->i < fractal->transformCommon.stopIterationsS)
	{
			...
			&& aux->i < fractal->transformCommon.stopIterationsM)
	{
		...
		z.x = mad(scaleM, z.x, -offsetM.x * (scaleM - 1.0));
		z.y = mad(scaleM, z.y, -offsetM.y * (scaleM - 1.0));
		z.w = mad(scaleM, z.w, -offsetM.w * (scaleM - 1.0));
		z.z -= 0.5 * offsetM.z * native_divide((scaleM - 1.0), scaleM);
		z.z = -fabs(-z.z);
		z.z += 0.5 * offsetM.z * native_divide((scaleM - 1.0), scaleM);
		z.z = scaleM * z.z;
		aux->DE *= scaleM * fractal->analyticDE.scale1;
