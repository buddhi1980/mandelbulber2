...
void MengerPrismShape2Iteration(float4 *z, __constant sFractalCl *fractal, sExtendedAuxCl *aux)
{
	if (fractal->transformCommon.functionEnabledSwFalse)
	{
		*z = (float4){-z->z, z->x, z->y, z->w};
	}
			...
			&& aux->i < fractal->transformCommon.stopIterationsP1)
	{
		...
		float tempOff = (fractal->transformCommon.offset0 + SQRT_3_4d2);
	...
	{ // CrossMengerTrick
		...
		float dot1 = (mad(z->x, -SQRT_3_4, z->y * 0.5f)) * fractal->transformCommon.scaleD1;
		...
		// else if ((z->y - 1.5f) * (z->y - 1.5f) + z->z * z->z < z->y * z->y + (z->z - 1.5f) * (z->z -
		// 1.5f))
		{
			dy = 1.5f;
		}
			...
			&& aux->i < fractal->transformCommon.stopIterationsB)
	{
		...
		float dot2 = (mad(z->x, -SQRT_3_4, z->y * 0.5f)) * fractal->transformCommon.scaleF1;
		float t = max(0.0f, dot2);
		...
		*z -= (float4){0.5f * SQRT_3, 1.5f, 1.5f, 0.0f};
...
#else
void MengerPrismShape2Iteration(double4 *z, __constant sFractalCl *fractal, sExtendedAuxCl *aux)
{
	if (fractal->transformCommon.functionEnabledSwFalse)
	{
		*z = (double4){-z->z, z->x, z->y, z->w};
	}
	...
	{ // CrossMengerTrick
		...
		// else if ((z->y - 1.5) * (z->y - 1.5) + z->z * z->z < z->y * z->y + (z->z - 1.5) * (z->z -
		// 1.5))
		{
			dy = 1.5;
		}
			...
			&& aux->i < fractal->transformCommon.stopIterationsB)
	{
		...
		*z -= (double4){0.5 * SQRT_3, 1.5, 1.5, 0.0};
