...
float4 GeneralizedFoldBoxIteration(float4 z, __constant sFractalCl *fractal, sExtendedAuxCl *aux)
{
	float3 zXYZ = z.xyz;
	int i;
	...
	float melt = fractal->mandelbox.melt;
	float solid = fractal->mandelbox.solid;
	...
	{ // mirror check
		int side_m = side;
		float3 Nv_m = Nv[side_m];
		float3 X_m = mad(-(dot(zXYZ, Nv_m) - solid), Nv_m, zXYZ);
		...
		if (side != -1)
		{ // rotation check
			float3 Xmr_intersect = mad(a, L, Y);
			int side_r = side;
			float3 Nv_r = Nv[side_r];
			...
			float a_rmin = native_divide((dot(zXYZ, L_r) - dot(Xmr_intersect, L_r)), (dot(L_r, L_r)));
...
#else
double4 GeneralizedFoldBoxIteration(double4 z, __constant sFractalCl *fractal, sExtendedAuxCl *aux)
{
	double3 zXYZ = z.xyz;
	int i;
	...
	{ // mirror check
		int side_m = side;
		double3 Nv_m = Nv[side_m];
		double3 X_m = mad(-(dot(zXYZ, Nv_m) - solid), Nv_m, zXYZ);
		...
		if (side != -1)
		{ // rotation check
			double3 Xmr_intersect = mad(a, L, Y);
			int side_r = side;
			double3 Nv_r = Nv[side_r];
			// The axis of rotation is define by the cross product of Nv_m and Nv_r and
			// the intersection of the line between Nv_m and Nv_r and  Xmr_intersect.
			double3 L_r = cross(Nv_m, Nv_r);
			// The closest point between z and the line of rotation can be found by minimizing
			// the square of the distance (D) between z and the line
			// X = mad(a_rmin., L_r, Xmr_intersect)
			// Setting dD/da_rmin equal to zero and solving for a_rmin.
			double a_rmin = native_divide((dot(zXYZ, L_r) - dot(Xmr_intersect, L_r)), (dot(L_r, L_r)));
			...
			{ // inversion check
				...
				double3 X_i = mad(a, L, Y);
				double3 z2X = X_i - zXYZ;
				// Is z above the melt layer.
				if (dot(z2X, z2X) > (melt * melt))
				{
					double z2X_mag = length(z2X);
					zXYZ += z2X * (2.0 * native_divide((z2X_mag - melt), (z2X_mag + .00000001)));
					aux->color += fractal->mandelbox.color.factor.z;
				}
			}
			else
