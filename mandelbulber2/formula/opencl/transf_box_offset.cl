...
void TransfBoxOffsetIteration(float4 *z, __constant sFractalCl *fractal, sExtendedAuxCl *aux)
{
	float4 temp = *z;
...
#else
void TransfBoxOffsetIteration(double4 *z, __constant sFractalCl *fractal, sExtendedAuxCl *aux)
{
	double4 temp = *z;
	z->x = copysign(fractal->transformCommon.additionConstant000.x, z->x) + z->x;
	z->y = copysign(fractal->transformCommon.additionConstant000.y, z->y) + z->y;
