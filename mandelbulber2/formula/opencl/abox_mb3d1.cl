...
REAL4 AboxMb3d1Iteration(REAL4 z, __global sFractalCl *fractal, sExtendedAuxCl *aux)
{
	...
	// x = Fold - abs(abs(x+FoldXM) - Fold) - abs(FoldXM)
	REAL fold = fractal->mandelbox.foldingValue;
	REAL foldXM = fractal->transformCommon.additionConstant000.x;
	REAL foldYM = fractal->transformCommon.additionConstant000.y;
	REAL foldZM = fractal->transformCommon.additionConstant000.z;
	...
	if (fractal->transformCommon.functionEnabledz && z.z != oldZ.z)
	...
	// else m = 1
	REAL rr = length(z) * length(z);
	REAL m = 1.0f;
	REAL minRSqr = fractal->mandelbox.foldingSphericalMin * fractal->mandelbox.foldingSphericalMin;
	...
	z *= actualScale;
	aux->DE = aux->DE * fabs(actualScale) + 1.0f;
