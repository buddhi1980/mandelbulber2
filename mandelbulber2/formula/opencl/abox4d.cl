...
DECIMAL4 Abox4dIteration(DECIMAL4 z, __constant sFractalCl *fractal, sExtendedAuxCl *aux)
{
	aux->actualScale = mad(
		(fabs(aux->actualScale) - 1.0f), fractal->mandelboxVary4D.scaleVary, fractal->mandelbox.scale);

	// parabolic = paraOffset + iter *slope + (iter *iter *scale)
	DECIMAL paraAddP0 = 0.0f;
	if (fractal->Cpara.enabledParabFalse)
	{
		DECIMAL parabScale = 0.0f;
			...
			&& aux->i < fractal->transformCommon.stopIterationsR)
	{
		...
		{
			tp = z;
			DECIMAL alpha = fractal->transformCommon.rotation44a.x * M_PI_180;
		...
		{
			tp = z;
			DECIMAL beta = fractal->transformCommon.rotation44a.y * M_PI_180;
		...
		{
			tp = z;
			DECIMAL gamma = fractal->transformCommon.rotation44a.z * M_PI_180;
		...
		{
			tp = z;
			DECIMAL delta = fractal->transformCommon.rotation44b.x * M_PI_180;
		...
		{
			tp = z;
			DECIMAL epsilon = fractal->transformCommon.rotation44b.y * M_PI_180;
		...
		{
			tp = z;
			DECIMAL zeta = fractal->transformCommon.rotation44b.z * M_PI_180;
	...
	DECIMAL scaleColor = fractal->foldColor.colorMin + fabs(aux->actualScale);
