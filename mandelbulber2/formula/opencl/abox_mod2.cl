...
void AboxMod2Iteration(float4 *z, __constant sFractalCl *fractal, sExtendedAuxCl *aux)
{
	...
	/*	float rr;
		if (temp > 0.0f)
			rr = mad(z->z, z->z, mad(z->x, z->x, z->y * z->y)); // on top & bottom of cyl. z->z should be
		tempZ
		else
			rr = mad(z->x, z->x, z->y * z->y); // on cyl body*/
	// cylinder half size
	float tempZ = fabs(z->z) - fractal->transformCommon.offset05;
	float rr = mad(z->x, z->x, z->y * z->y);
...
#else
void AboxMod2Iteration(double4 *z, __constant sFractalCl *fractal, sExtendedAuxCl *aux)
{
