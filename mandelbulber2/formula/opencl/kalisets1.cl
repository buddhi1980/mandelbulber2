...
float4 Kalisets1Iteration(float4 z, __constant sFractalCl *fractal, sExtendedAuxCl *aux)
{
	float4 c = aux->const_c;

	z = fabs(z);
	float sqs = (z.x * z.x + mad(z.y, z.y, z.z * z.z) + 1e-21f); // sph inv
	float m;
	float minR = fractal->transformCommon.minR0; //  KaliDucks
...
#else
double4 Kalisets1Iteration(double4 z, __constant sFractalCl *fractal, sExtendedAuxCl *aux)
{
	double4 c = aux->const_c;

	z = fabs(z);
	double sqs = (z.x * z.x + mad(z.y, z.y, z.z * z.z) + 1e-21); // sph inv
	double m;
	double minR = fractal->transformCommon.minR0; //  KaliDucks

	if (sqs < minR)
		m = 1.0 / native_sqrt(minR);
	else
		m = native_divide(fractal->transformCommon.scale, sqs); // kalisets
