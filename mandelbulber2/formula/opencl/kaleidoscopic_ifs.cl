...
void KaleidoscopicIfsIteration(float4 *z, __constant sFractalCl *fractal, sExtendedAuxCl *aux)
{
...
#else
void KaleidoscopicIfsIteration(double4 *z, __constant sFractalCl *fractal, sExtendedAuxCl *aux)
{
	if (fractal->IFS.absX) z->x = fabs(z->x);
	if (fractal->IFS.absY) z->y = fabs(z->y);
	if (fractal->IFS.absZ) z->z = fabs(z->z);
