...
void RiemannBulbMsltoeMod2Iteration(float4 *z, __constant sFractalCl *fractal, sExtendedAuxCl *aux)
{
	Q_UNUSED(aux);

	float rad2 = fractal->transformCommon.minR05;
	...
	if (r2 < rad2 * rad2)
	{
		if (fractal->transformCommon.functionEnabled)
			// smooth inside
			*z *=
				rad2 * ((r2 * 0.1f) + 0.4f) * 1.18f * native_divide(fractal->transformCommon.scaleA1, r2);
		else
		{
...
#else
void RiemannBulbMsltoeMod2Iteration(double4 *z, __constant sFractalCl *fractal, sExtendedAuxCl *aux)
{
	Q_UNUSED(aux);
	...
	if (r2 < rad2 * rad2)
	{
		if (fractal->transformCommon.functionEnabled)
			// smooth inside
			*z *= rad2 * ((r2 * 0.1) + 0.4) * 1.18 * native_divide(fractal->transformCommon.scaleA1, r2);
		else
		{
			*z *= fractal->transformCommon.constantMultiplier111;
		}
