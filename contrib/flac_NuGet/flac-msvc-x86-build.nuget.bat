...
copy %INITDIR%\flac-msvc-%tbs_arch%.targets %BINDIR%\build\native\flac-msvc-%tbs_arch%.targets
copy %INITDIR%\flac-msvc-%tbs_arch%.nuspec %BINDIR%\flac-msvc-%tbs_arch%.nuspec
copy %SRC%\COPYING %BINDIR%\COPYING
copy %SRC%\README %BINDIR%\README
cd %BUILDTREE%
nuget pack %BINDIR%\flac-msvc-%tbs_arch%.nuspec
cd %INITDIR%
REM --- exit ----
GOTO:eof
