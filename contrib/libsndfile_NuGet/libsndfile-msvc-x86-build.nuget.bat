...
REM OPUS (OPUS_LIBRARY OPUS_INCLUDE_DIR)
SET VER=1.3.1.7707
set OPUSDIR=%BUILDTREE%\deps\opus-msvc-%tbs_arch%.%VER%\build\native
nuget install opus-msvc-%tbs_arch% -Version %VER%
...
rd /s /q %BUILDTREE%\libsndfile
mkdir %BUILDTREE%\libsndfile
cd %BUILDTREE%\libsndfile
...
-DFLAC_LIBRARY=%FLACDIR%\lib_release\FLAC.lib ^
-DFLAC_INCLUDE_DIR=%FLACDIR%\include ^
-DOPUS_LIBRARY=%OPUSDIR%\lib_release\opus.lib ^
-DOPUS_INCLUDE_DIR=%OPUSDIR%\include ^
...
-DVorbis_Enc_INCLUDE_DIR=%VorbisDIR%\include ^
-DVorbis_File_INCLUDE_DIR=%VorbisDIR%\include ^
-DFLAC_LIBRARY=%FLACDIR%\lib_debug\FLAC.lib ^
-DFLAC_INCLUDE_DIR=%FLACDIR%\include ^
-DOPUS_LIBRARY=%OPUSDIR%\lib_debug\opus.lib ^
-DOPUS_INCLUDE_DIR=%OPUSDIR%\include ^
-DCMAKE_INSTALL_PREFIX=%BINDIR%\build\native\ ^
...
copy %INITDIR%\libsndfile-msvc-%tbs_arch%.targets %BINDIR%\build\native\libsndfile-msvc-%tbs_arch%.targets
copy %INITDIR%\libsndfile-msvc-%tbs_arch%.nuspec %BINDIR%\libsndfile-msvc-%tbs_arch%.nuspec
copy %SRC%\COPYING %BINDIR%\COPYING
copy %SRC%\README %BINDIR%\README
cd %BUILDTREE%
nuget pack %BINDIR%\libsndfile-msvc-%tbs_arch%.nuspec
cd %INITDIR%
REM --- exit ----
GOTO:eof
